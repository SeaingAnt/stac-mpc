...
class DroneV1(environment.Environment):
    ...
    def default_params(self) -> EnvParams:
        """Default environment parameters for Drone-v1."""
        return EnvParams()
    ...
    def step_env(
        ...
        single_obs = self._single_obs(state_next, env_state.target_yaw, env_state.target_pos)
        # env_state.history shape: (seq_len, obs_dim)
        new_history = jnp.concatenate([env_state.history[1:], single_obs[None, :]], axis=0)
    ...
    def reset_env(
        ...
        single_obs = self._single_obs(state, target_yaw, target_pos)
    ...
    def get_obs(self, env_state: EnvState) -> jax.Array:
        ...
        return self._single_obs(env_state, env_state.target_yaw, env_state.target_pos)

    def _single_obs(self, state: ModelState, target_yaw: float, target_pos: jax.Array) -> jax.Array:
        ...
        current_yaw = jnp.arctan2(2.0 * (q[0] * q[3] + q[1] * q[2]), 1.0 - 2.0 * (q[2]**2 + q[3]**2))
        yaw_error = jnp.arctan2(jnp.sin(current_yaw - target_yaw), jnp.cos(current_yaw - target_yaw))
