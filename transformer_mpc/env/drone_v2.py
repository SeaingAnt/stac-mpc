...
class EnvParams(environment.EnvParams):
    ...
    obstacle_radius: float = struct.field(pytree_node=False, default=0.2)


class DroneV2(environment.Environment):
    """JAX Compatible version of mikrokopter drone converging to a target."""

    def __init__(self):
        self.type = ""
        self.state_dim = 13
        self.obs_dim = 17  # Added yaw_error (1) and to_obstacle (3)
        self.action_dim = self.action_space(self.default_params).shape[0]
        self.param_dim = 8  # len(jax.tree.leaves(self.default_params))
        self.target_pos = jnp.array([0.0, 1.0, 0.5])
        self.obstacle_pos = jnp.array([0.0, 0.0, 0.5])
        self.pos = jnp.array([0.0, -1.0, 0.5])
    ...
    def default_params(self) -> EnvParams:
        """Default environment parameters for Drone-v2."""
        return EnvParams()
    ...
    class mpc_dynamics:
        ...
        #def constraint_cost(self, x, u, t, base_env_state, params):
        #    pos = x[:3]
        #    obstacle_pos = base_env_state[:3]
        #    diff = pos - obstacle_pos
        #    dist = jnp.sqrt(jnp.sum(diff**2) + 1e-6)
        #
        #    # Penalize when drone is closer than obstacle_radius
        #    violation = jax.nn.softplus(10 * (params.obstacle_radius - dist)) / 10
        #    return 200.0 * violation**2
    ...
    def step_env(
        ...
        current_yaw = jnp.arctan2(
            2.0 * (q[0] * q[3] + q[1] * q[2]), 1.0 - 2.0 * (q[2] ** 2 + q[3] ** 2)
        )
        yaw_error = jnp.abs(
            jnp.arctan2(
                jnp.sin(current_yaw - env_state.target_yaw),
        ...
        reward_yaw = jax.lax.select(
            ...
            jnp.exp(-1.5 * yaw_error),
        )

        thrust_normalized_input = action_rescaled.at[0].set(
            action_rescaled[0] / (params.max_input[0])
        )
        thrust_normalized_input_last = env_state.last_u.at[0].set(
            env_state.last_u[0] / (params.max_input[0])
        )
        ...
        smooth_score = jnp.exp(
            -1.5
            * jnp.linalg.norm(thrust_normalized_input - thrust_normalized_input_last)
        )
        ...
        reward = (
            ...
            + 0.6 * pos_score * reward_yaw * omega_score
        ...
        # Update history: drop oldest, append new per-step observation
        single_obs = self._single_obs(
            state_next,
            env_state.target_yaw,
            env_state.target_pos,
            env_state.obstacle_pos,
        )
        # env_state.history shape: (seq_len, obs_dim)
        new_history = jnp.concatenate(
            [env_state.history[1:], single_obs[None, :]], axis=0
        )
        ...
        env_state = EnvState(
            ...
            obstacle_pos=env_state.obstacle_pos,
    ...
    def _step_dynamics(
        ...
        vel = state.vel + params.dt * (orientation_mat @ acc[:3] + noise_state[0:3])
        omega = state.omega + params.dt * (acc[3:6] + noise_state[3:6])
    ...
    def reset_env(
        ...
        key, key_obstacle, key_target, key_yaw = jax.random.split(key, 4)
        ...
        target_yaw = jax.lax.select(
            params.is_testing,
            jnp.array(0.0),  # Point forward when testing
            jax.random.uniform(key_yaw, minval=-jnp.pi, maxval=jnp.pi),
        )

        # Initialize per-step observation and history buffer
        single_obs = self._single_obs(state, target_yaw, target_pos, obstacle_pos)
        seq_len = params.mpc_horizon + 1
        history = jnp.tile(single_obs[None, :], (seq_len, 1))
    ...
    def get_obs(
        self, env_state: EnvState | None = None, key: jax.Array | None = None
    ) -> jax.Array:
        """Return the sequence of past per-step observations (history).

        Returns a 2D array with shape (seq_len, obs_dim) where seq_len == params.mpc_horizon+1.
        """
        # If history exists, return it directly (seq_len, obs_dim)
        if hasattr(env_state, "history"):
            return env_state.history

        # Fallback for legacy EnvState: return single-step observation
    ...
    def _single_obs(
        ...
        """Compute single-step observation vector from a ModelState and target yaw."""
        pos = state.pos
        vel = state.vel
        ...
        yaw_error = jnp.arctan2(
            jnp.sin(current_yaw - target_yaw), jnp.cos(current_yaw - target_yaw)
        )
    ...
    def is_terminal(self, state: EnvState, params: EnvParams) -> jax.Array:
        ...
        collision = (
            jnp.linalg.norm(state.pos - state.obstacle_pos) <= params.obstacle_radius
        )
        return jnp.logical_or(time_limit, collision)

    def eval_metrics(
        self, env_state: EnvState, params: EnvParams
    ) -> dict[str, jax.Array]:
    ...
    def discount(self, state: EnvState, params: EnvParams) -> jax.Array:
        """Return a discount of 0.0 for collision (termination), 1.0 for time limit (truncation)."""
        collision = (
            jnp.linalg.norm(state.pos - state.obstacle_pos) <= params.obstacle_radius
        )
    ...
    def observation_space(self, params: EnvParams) -> spaces.Box:
        ...
        seq_len = params.mpc_horizon + 1
        return spaces.Box(-1, 1, shape=(seq_len, self.obs_dim), dtype=jnp.float32)
    ...
    def render_frame_from_state(
        ...
        # Draw arms
        ax.plot(
            [pos[0], p1[0]], [pos[1], p1[1]], [pos[2], p1[2]], color="k", linewidth=2
        )
        ax.plot(
            [pos[0], p2[0]], [pos[1], p2[1]], [pos[2], p2[2]], color="k", linewidth=2
        )
        ax.plot(
            [pos[0], p3[0]], [pos[1], p3[1]], [pos[2], p3[2]], color="k", linewidth=2
        )
        ax.plot(
            [pos[0], p4[0]], [pos[1], p4[1]], [pos[2], p4[2]], color="k", linewidth=2
        )
        ...
        # Obstacle
        params = self.default_params
        if hasattr(env_state, "obstacle_pos"):
            obstacle_pos = np.asarray(jax.device_get(env_state.obstacle_pos))
            obstacle_radius = float(params.obstacle_radius)
            u, v = np.mgrid[0 : 2 * np.pi : 20j, 0 : np.pi : 10j]
            x = obstacle_pos[0] + obstacle_radius * np.cos(u) * np.sin(v)
            y = obstacle_pos[1] + obstacle_radius * np.sin(u) * np.sin(v)
            z = obstacle_pos[2] + obstacle_radius * np.cos(v)
    ...
    def plot_rollouts(series_list: List[dict], output: Path | None = None) -> None:
        ...
        for series in series_list:
            ...
            # Obstacle
            params = EnvParams()
            obstacle_pos = np.asarray(series["obstacle_pos"])
            obstacle_radius = float(params.obstacle_radius)
            u, v = np.mgrid[0 : 2 * np.pi : 20j, 0 : np.pi : 10j]
            x = obstacle_pos[0] + obstacle_radius * np.cos(u) * np.sin(v)
            y = obstacle_pos[1] + obstacle_radius * np.sin(u) * np.sin(v)
            z = obstacle_pos[2] + obstacle_radius * np.cos(v)
            ax_path.plot_surface(x, y, z, color="red", alpha=0.3)
        ...
        if distance_series or yaw_series:
            ...
            if distance_series:
                ...
                reference_dt = (
                    distance_series[0]["dt"] if distance_series else default_dt
                )
            ...
            if yaw_series:
                ...
                padded_yaw = np.array(
                    [
                        np.pad(
                            y["values"],
                            (0, max_len_yaw - y["values"].shape[0]),
                            mode="edge",
                        )
                        for y in yaw_series
                    ]
                ...
                reference_dt = (
                    distance_series[0]["dt"] if distance_series else yaw_series[0]["dt"]
                )
                ...
                ax_yaw.plot(
                    timesteps_yaw, mean_yaw, color="red", label="Yaw error", linewidth=2
                )
                ax_yaw.fill_between(
                    timesteps_yaw,
                    mean_yaw - std_yaw,
